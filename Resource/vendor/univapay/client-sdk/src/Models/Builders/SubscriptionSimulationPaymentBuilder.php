<?php

declare(strict_types=1);

/*
 * UnivapayClientSdk
 *
 * This file was automatically generated for Univapay by APIMATIC v3.0 ( https://www.apimatic.io ).
 */

namespace UnivaPay\Models\Builders;

use Core\Utils\CoreHelper;
use UnivaPay\Models\SubscriptionSimulationPayment;

/**
 * Builder for model SubscriptionSimulationPayment
 *
 * @see SubscriptionSimulationPayment
 */
class SubscriptionSimulationPaymentBuilder
{
    /**
     * @var SubscriptionSimulationPayment
     */
    private $instance;

    private function __construct(SubscriptionSimulationPayment $instance)
    {
        $this->instance = $instance;
    }

    /**
     * Initializes a new Subscription Simulation Payment Builder object.
     */
    public static function init(): self
    {
        return new self(new SubscriptionSimulationPayment());
    }

    /**
     * Sets due date field.
     *
     * @param \DateTime|null $value
     */
    public function dueDate(?\DateTime $value): self
    {
        $this->instance->setDueDate($value);
        return $this;
    }

    /**
     * Sets zone id field.
     *
     * @param string|null $value
     */
    public function zoneId(?string $value): self
    {
        $this->instance->setZoneId($value);
        return $this;
    }

    /**
     * Sets amount field.
     *
     * @param int|null $value
     */
    public function amount(?int $value): self
    {
        $this->instance->setAmount($value);
        return $this;
    }

    /**
     * Sets currency field.
     *
     * @param string|null $value
     */
    public function currency(?string $value): self
    {
        $this->instance->setCurrency($value);
        return $this;
    }

    /**
     * Sets is paid field.
     *
     * @param bool|null $value
     */
    public function isPaid(?bool $value): self
    {
        $this->instance->setIsPaid($value);
        return $this;
    }

    /**
     * Sets is last payment field.
     *
     * @param bool|null $value
     */
    public function isLastPayment(?bool $value): self
    {
        $this->instance->setIsLastPayment($value);
        return $this;
    }

    /**
     * Sets successful payment date field.
     *
     * @param \DateTime|null $value
     */
    public function successfulPaymentDate(?\DateTime $value): self
    {
        $this->instance->setSuccessfulPaymentDate($value);
        return $this;
    }

    /**
     * Unsets successful payment date field.
     */
    public function unsetSuccessfulPaymentDate(): self
    {
        $this->instance->unsetSuccessfulPaymentDate();
        return $this;
    }

    /**
     * Sets terminate with status field.
     *
     * @param string|null $value
     */
    public function terminateWithStatus(?string $value): self
    {
        $this->instance->setTerminateWithStatus($value);
        return $this;
    }

    /**
     * Unsets terminate with status field.
     */
    public function unsetTerminateWithStatus(): self
    {
        $this->instance->unsetTerminateWithStatus();
        return $this;
    }

    /**
     * Sets retry interval field.
     *
     * @param string|null $value
     */
    public function retryInterval(?string $value): self
    {
        $this->instance->setRetryInterval($value);
        return $this;
    }

    /**
     * Unsets retry interval field.
     */
    public function unsetRetryInterval(): self
    {
        $this->instance->unsetRetryInterval();
        return $this;
    }

    /**
     * Add an additional property to this model.
     *
     * @param string $name Name of property.
     * @param mixed $value Value of property.
     */
    public function additionalProperty(string $name, $value): self
    {
        $this->instance->addAdditionalProperty($name, $value);
        return $this;
    }

    /**
     * Initializes a new Subscription Simulation Payment object.
     */
    public function build(): SubscriptionSimulationPayment
    {
        return CoreHelper::clone($this->instance);
    }
}
